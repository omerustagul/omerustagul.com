...
import { Drawer, Field, MkSelect } from "@/components/admin/ui";
...
function Thumb({ f, big }: { f: any; big?: boolean }) {
  return (
    <div className="ph" style={{ aspectRatio: big ? "4/3" : "1/1", borderRadius: "var(--radius-sm)" }}>
      {f.src ? (
        <img src={f.src} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: "var(--radius-sm)" }} />
      ) : (
...
export function MediaClient() {
  const [folders, setFolders] = useState(MEDIA_FOLDERS_INIT);
  const [files, setFiles] = useState<any[]>(MEDIA_FILES_INIT);
  ...
  const [expanded, setExpanded] = useState<Set<string | null>>(() => new Set(MEDIA_FOLDERS_INIT.map((f) => f.id)));
  const inputRef = useRef<HTMLInputElement>(null);

  const childrenOf = (pid: string | null) => folders.filter((f) => (f.parent || null) === (pid === "all" ? null : pid));
  const fileCount = (id: string | null) => (id === "all" ? files.length : files.filter((f) => f.folder === id).length);
  const folderName = (id: string | null) => (id === "all" ? "Tüm medya" : (folders.find((f) => f.id === id) || {}).name || "—");
  const pathOf = (id: string | null) => {
    const out: any[] = [];
    let cur: any = folders.find((f) => f.id === id);
    while (cur) {
      out.unshift(cur);
  ...
  const indentedLabel = (id: string | null) => pathOf(id).map((f) => f.name).join("  ›  ");
  ...
  return (
    <div className="media" style={{ height: "calc(100vh - 120px)" }}>
      <aside className="media-side">
        <button className={`media-fld ${active === "all" ? "on" : ""}`} onClick={() => setActive("all")}>
          <span className="media-fld__tw" style={{ opacity: 0 }}>▸</span>
          <Icon name="media" size={16} /> <span className="media-fld__nm">Tüm medya</span> <span className="c">{fileCount("all")}</span>
        </button>
        ...
        {adding ? (
          <div className="media-newfld">
            ...
            />
            <button className="adm-iconbtn" onClick={addFolder}>
              <Icon name="check" size={14} />
            </button>
          </div>
        ) : (
          <button className="media-fld media-fld--add" onClick={() => setAdding(true)}>
            <span className="media-fld__tw" style={{ opacity: 0 }}>▸</span>
            <Icon name="plus" size={15} /> {active === "all" ? "Yeni klasör" : "Alt klasör ekle"}
          </button>
        )}
      </aside>
      ...
      <div className="media-main">
        <nav className="media-crumb">
          ...
          {pathOf(active).map((f) => (
            <React.Fragment key={f.id}>
              ...
              <button className={f.id === active ? "on" : ""} onClick={() => setActive(f.id)}>
                {f.name}
              </button>
            </React.Fragment>
          ))}
        ...
        <div className="media-bar">
          <div>
            <b style={{ fontWeight: 600 }}>{folderName(active)}</b>{" "}
            <span style={{ color: "var(--text-muted)", fontFamily: "var(--font-mono)", fontSize: "var(--fs-xs)" }}>
              · {subfolders.length} klasör · {visible.length} dosya
            </span>
          </div>
          <div className="media-search" style={{ display: "flex", alignItems: "center", gap: 6, background: "var(--surface)", padding: "0.4rem 0.6rem", borderRadius: "6px", border: "1px solid var(--border)" }}>
            <Icon name="search" size={15} />
            <input placeholder="Dosya ara…" value={q} onChange={(e) => setQ(e.target.value)} style={{ border: "none", background: "none", outline: "none", fontSize: 13, width: 140 }} />
          </div>
          <div className="adm-seg">
            <button className={view === "grid" ? "on" : ""} onClick={() => setView("grid")}>Izgara</button>
            <button className={view === "list" ? "on" : ""} onClick={() => setView("list")}>Liste</button>
          </div>
          <button className="adm-btn adm-btn--ghost" onClick={() => setAdding(true)}>
            <Icon name="plus" size={15} /> Klasör
          </button>
          <button className="adm-btn adm-btn--primary" onClick={() => inputRef.current?.click()}>
            <Icon name="plus" size={15} /> Yükle
          </button>
        ...
        >
          {subfolders.length > 0 && !q && (
            <div className="media-grid" style={{ marginBottom: "2rem" }}>
              {subfolders.map((f) => (
                <button key={f.id} className="folder-card" onClick={() => setActive(f.id)}>
          ...
          {visible.length === 0 && subfolders.length === 0 ? (
            <div className="adm-empty">
              <h3>Bu klasör boş</h3>
              <p>Alt klasör ekleyin ya da dosyaları buraya sürükleyip bırakın.</p>
            </div>
          ) : view === "grid" ? (
            <div className="media-grid">
              {visible.map((f) => (
                <button key={f.id} className="file-card" onClick={() => setSel(f)}>
                  <Thumb f={f} />
                  <div className="file-card__nm">{f.name}</div>
                  <div className="file-card__meta">{f.type} · {f.size}</div>
                </button>
              ))}
            </div>
          ) : (
              ...
              <tbody>
                {visible.map((f) => (
                  <tr key={f.id} onClick={() => setSel(f)} style={{ cursor: "pointer" }}>
                    ...
                    <td>
                      <button className="adm-iconbtn" onClick={(e) => { e.stopPropagation(); setFiles((p) => p.filter((x) => x.id !== f.id)); }}>
      ...
      {sel && (
        <MediaDetail
          ...
          onClose={() => setSel(null)}
          onChange={(u: any) => { setFiles((p) => p.map((x) => (x.id === u.id ? u : x))); setSel(u); }}
...
function MediaDetail({ file, folders, pathLabel, onClose, onChange, onDelete }: any) {
    ...
    <Drawer
      ...
      footer={
        <>
          ...
          <button className="adm-btn adm-btn--danger" onClick={onDelete}><Icon name="trash" size={14} /> Sil</button>
        </>
      }
    >
      <div style={{ borderRadius: "var(--radius)", overflow: "hidden", border: "1px solid var(--border)", marginBottom: "1.5rem" }}>
        <Thumb f={file} big />
      </div>
      <Field label="Dosya adı">
        <input className="adm-input" value={file.name} onChange={(e) => onChange({ ...file, name: e.target.value })} />
      </Field>
      <Field label="Klasör (taşı)">
        <MkSelect
          value={file.folder}
          onChange={(v: string) => onChange({ ...file, folder: v })}
          options={folders.map((f: any) => ({ value: f.id, label: pathLabel(f.id) }))}
        />
      </Field>
      <div className="set-row" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "1rem" }}>
      ...
      <Field label="Genel URL">
        <div style={{ display: "flex", gap: "6px" }}>
          <input className="adm-input" style={{ flex: 1 }} readOnly value={`https://cdn.marka.studio/${file.folder}/${file.name}`} />
          <button className="adm-btn adm-btn--ghost" onClick={() => { setCopied(true); setTimeout(() => setCopied(false), 1500); }}>

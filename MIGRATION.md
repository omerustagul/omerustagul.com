...
- [x] **F3 — Tasarım sistemi:** ✅ Çekirdek primitive'ler TSX'e taşındı (`web/src/components/ui/`: Button, Logo, Badge, Tag, SectionHeading, Rating, Input, Select) — CSS tek `components.css`'te (SSR-dostu). `theme.js` runtime → `ThemeProvider` + `useTheme` (`web/src/components/theme/`), `lib/theme.ts` (kataloglar + applyTheme), FOUC önleyici `public/theme-init.js`. Canlı tema paneli + gezilebilir `/showcase`. _Kalan (F3 devamı): Avatar, Media, Marquee, IconButton ve 4 kart (ProjectCard/CourseCard/BlogCard/ProductCard)._

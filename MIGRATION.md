...
- [x] **F4 — Site (okuma) [v1]:** ✅ Gerçek anasayfa Server Component olarak Prisma'dan okuyor: SiteHeader (sticky+blur nav), Hero (istatistikler), Partner marquee, Öne çıkan işler (ProjectCard), Hizmetler, Akademi (CourseCard), Market (ProductCard), Blog (BlogCard), CTA, SiteFooter. Eksik içerik modelleri eklendi (`Project`, `BlogPost`, `Partner`, `Service`) → migration `20260618051723_content_models` + seed (6 proje, 4 yazı, 7 partner, 4 hizmet). `dynamic = "force-dynamic"`. Playwright ile doğrulandı. _Kalan (F4 devamı): `pages/*` ikincil sayfalar (proje/kurs/ürün/blog detay), WeeklyWork/Collections/Games/Stats bölümleri._

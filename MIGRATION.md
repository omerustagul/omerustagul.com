...
- [x] **F2 — Auth:** ✅ NextAuth v5 (`next-auth@beta`) Credentials + JWT; bcrypt hash (`src/lib/password.ts`); split-config (`auth.config.ts` edge-safe / `auth.ts` Node); `role` (ADMIN/EDITOR/MEMBER) JWT'de taşınıyor; `/admin` **proxy.ts** (Next 16 konvansiyonu) ile korunuyor; login sayfası + register/login/logout server action'ları. Uçtan uca Playwright ile doğrulandı (kimliksiz→/login, admin girişi→/admin, çıkış→tekrar /login). Admin: `admin@marka.test / admin1234`.
  > Not: `trustHost: true` self-host/Vercel için gerekli. `jwt`+`session` callback'leri `authConfig`'te (proxy de `role`'ü görsün diye). `middleware.ts` Next 16'da deprecated → `proxy.ts` (default `proxy` fonksiyon export'u).
- [x] **F3 — Tasarım sistemi:** ✅ **Tüm** primitive'ler TSX'e taşındı (`web/src/components/ui/`: Button, Logo, Badge, Tag, SectionHeading, Rating, Input, Select, Avatar/AvatarStack, IconButton, Media, Marquee + kartlar `cards/`: ProjectCard, CourseCard, ProductCard, BlogCard) — CSS tek `components.css`'te (SSR-dostu). `theme.js` runtime → `ThemeProvider` + `useTheme` (`web/src/components/theme/`), `lib/theme.ts` (kataloglar + applyTheme), FOUC önleyici `public/theme-init.js`. Canlı tema paneli + gezilebilir `/showcase` (Playwright ile açık/koyu + vurgu doğrulandı).
- [x] **F4 — Site (okuma):** ✅ Anasayfa + tüm listeleme/detay sayfaları Prisma'dan okuyor. `(site)` route group + paylaşılan `SiteHeader`/`SiteFooter`. Sayfalar: `/` (Hero, partner marquee, işler, hizmetler, akademi, market, blog, **Stats** canlı sayımlar, CTA), `/projects` + `/projects/[slug]` (problem/çözüm + oy), `/academy` + `/academy/[slug]` (müfredat + EnrollButton), `/market` + `/market/[slug]` (yorumlar + BuyButton), `/blog` + `/blog/[slug]` (okuma ilerleme çubuğu), `/iletisim` (lead formu → server action). İçerik modelleri (`Project/BlogPost/Partner/Service`) + migration `content_models` + seed. Ticaret köprüsü: `enrollCourse`/`buyProduct`/`createLead` server action'ları. Playwright ile doğrulandı.
- [x] **F5 — Admin (yazma):** ✅ `/admin` paneli (sidebar layout, proxy ile ADMIN/EDITOR korumalı). Dashboard (canlı sayımlar + son talepler) + tam CRUD: Projeler, Blog (taslak/yayında), Kurslar, Ürünler, Hizmetler, Partnerler — hepsi Server Actions (`lib/actions/admin.ts`, `requireAdmin` koruması) + `revalidatePath` ile **canlı yansıma**. Talepler (durum + sil), Ayarlar (site teması → `SiteSettings`). Playwright ile doğrulandı: admin'den eklenen proje anında `/projects`'te göründü. _Kalan: kurs modül/ders editörü, medya yükleme (F7), kullanıcı yönetimi._
- [x] **F6 — Domain özellikleri:** ✅ Kurs oynatıcı (`/academy/[slug]/learn`, kayıt korumalı) — ders listesi, tamamla/geri al (`LessonProgress`), ilerleme çubuğu, %100'de sertifika. Proje oylama (`VoteButton` + `toggleVote`, `ProjectVote`). Refleks oyunu (`/oyunlar`) — skor + günlük hak limiti (`GameScore`) + canlı liderlik (groupBy). Randevu (`/randevu` → `Booking`) + admin yönetimi. Market satın alma F4'te köprülendi. Playwright ile doğrulandı (liderlik gerçek veriyle). _Kalan: 3 oyunun diğer ikisi (hafıza/sıralı), market yorum ekleme formu._
- [x] **F7 — AI + i18n + medya:** ✅ **AI:** sunucu tarafı blog taslağı (`lib/ai.ts`, Anthropic SDK `claude-opus-4-8`, `ANTHROPIC_API_KEY` yoksa zarif simülasyon) → `/admin/blog/ai` taslak üretir + düzenleyici açar. **i18n:** cookie tabanlı tr/en (`lib/i18n.ts` client-güvenli + `i18n-server.ts`), header'da `LanguageSwitcher` — Playwright ile EN/TR doğrulandı (de/ar genişletilebilir). **Medya:** `/admin/media` upload (`public/uploads`, admin korumalı, üretimde Blob/S3 notu). _Kalan: AI raporlar, içerik gövdesi tam i18n._
- [x] **F8 — Sertleştirme:** ✅ **SEO:** `sitemap.ts` (statik + dinamik URL'ler), `robots.ts` (admin/api disallow), kök metadata (OpenGraph/Twitter + title template), detay sayfalarında `generateMetadata`. **Rate-limit:** in-memory limiter (`lib/rate-limit.ts`) → lead/booking form'larına uygulandı (üretimde Redis notu). **Güvenlik başlıkları:** `next.config.ts` (X-Frame-Options, X-Content-Type, Referrer-Policy, Permissions-Policy, HSTS). **a11y:** "İçeriğe atla" skip-link + landmark'lar + focus ring'ler. **Ödeme:** `lib/payment.ts` soyutlaması (dev mock auto-approve; üretimde Stripe/iyzico) → enroll/buy akışına bağlandı. **404** sayfası. Playwright/curl ile doğrulandı.

---

### 🎉 Migration tamamlandı
Tüm fazlar (F0–F8) tamamlandı, build yeşil, [omerustagul/omerustagul.com](https://github.com/omerustagul/omerustagul.com) `main`'e push edildi. Üretime çıkış için kalanlar: gerçek `ANTHROPIC_API_KEY` + `PAYMENT_PROVIDER` anahtarları, object storage (Blob/S3), Vercel'de env vars, ve admin şifresinin değiştirilmesi.

  ...
  return (
    <div className="adm-field">
      <label style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>{block.label}
        {block.ai && <button className="adm-btn adm-btn--ghost" style={{ padding: ".3rem .7rem" }} disabled={aiBusy} onClick={onAI}>{aiBusy ? <><span className="ai-spinner" style={{ borderTopColor: "transparent", borderColor: "var(--accent)" }} /> …</> : <><Icon name="ai" size={13} /> AI ile yaz</>}</button>}
      </label>
...
function BlogWizard({ onClose, onSave }) {
  ...
  const generate = async (tplId) => {
    ...
    t.blocks.forEach((b, i) => {
      ...
      else if (b.t === "kicker") f[b.k] = kicker;
      else if (b.t === "lead") f[b.k] = lead;
      else if (b.t === "rich") f[b.k] = body;
      else if (b.t === "cover" || b.t === "image") f[b.k] = genImg(i * 60 + 20);
      else if (b.t === "gallery") f[b.k] = [0, 1, 2, 3].map(g => ({ id: Date.now() + g + Math.random(), src: genImg(g * 70), caption: "" }));
      else if (b.t === "qa") f[b.k] = [{ id: 1, q: `${topic} nedir?`, a: lead || "…" }, { id: 2, q: "Neden önemli?", a: "Markanın dijitalde fark yaratması için kritik." }];
      else if (b.t === "list") f[b.k] = [{ id: 1, h: "Net bir yön belirleyin", text: lead, img: genImg(30) }, { id: 2, h: "Tutarlı uygulayın", text: "Tek bir vurgu ve ritim.", img: null }];
    });

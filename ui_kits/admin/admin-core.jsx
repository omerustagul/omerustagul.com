...
function MkSelect({ value, onChange, options = [], placeholder = "Seçin", width, searchable, multi }) {
  ...
  const btnLabel = multi
    ? (sel.length === 0 ? placeholder : sel.length <= 2 ? opts.filter(o => sel.includes(o.value)).map(o => o.label).join(", ") : `${sel.length} hizmet seçili`)
    ...
    <div className={`mk-select ${open ? "open" : ""}`} ref={ref} style={width ? { width } : null}>
      ...
      {open && (
        <div className={`mk-select__pop ${up ? "up" : ""}`} role="listbox">
          ...
          {filtered.map((o, i) => {
            const header = o.group && o.group !== (filtered[i - 1] && filtered[i - 1].group)
              ? <div key={"g" + o.group} className="mk-select__group">{o.group}</div> : null;
            return (
              <React.Fragment key={o.value}>
                {header}
                <button type="button" role="option" aria-selected={isOn(o.value)}
                  className={`mk-select__opt ${isOn(o.value) ? "on" : ""} ${o.group ? "is-sub" : ""}`}
                  onClick={() => choose(o.value)}>
                  {multi && <span className={`mk-select__box ${isOn(o.value) ? "on" : ""}`}>{isOn(o.value) && <Icon name="ai" size={11} />}</span>}
                  <span>{o.label}</span>{!multi && isOn(o.value) && <span className="ck"><Icon name="ai" size={13} /></span>}
                </button>
              </React.Fragment>
            );
          })}
